...
export default function ExistingBusLinePopup({
  existingLine,
}: ExistingBusLinePopupProps) {
  return (
    <Popup>
      <h1 className={"text-lg"}>Buslinie {existingLine.properties!.ref}</h1>
      <br />
      <span className={"font-bold"}>Start:</span>{" "}
      {existingLine.properties!.from}
      <br />
      <span className={"font-bold"}>Ende:</span> {existingLine.properties!.to}
      <br />
      <span className={"font-bold"}>Betreiber:</span>{" "}
      {existingLine.properties!.operator}
      <br />
      <br />
      <a
