...
export default function ExistingBusNetwork({ network }: Props) {
  ...
  const renderedLines = busLines.map((feature) => (
    <ExistingBusLine key={feature.properties["@id"]} existingLine={feature} />
  ));
  const renderedStops = stops.map((feature) => (
    <ExistingBusStop key={feature.properties["@id"]} existingStop={feature} />
  ));
